...
class PDFReportGenerator:
    ...
    def _generate_cover(self, target: str, scan_info: Dict) -> str:
        ...
        <div class="cover">
            ...
            <div class="subtitle"><strong>{html.escape(target)}</strong></div>
            <div class="date">Generated: {date}</div>
            <div class="date" style="margin-top: 60px;">Prepared by: {html.escape(company)}</div>
        </div>
        """
    ...
    def _generate_findings_summary_table(self, findings: List[Dict]) -> str:
        ...
        for i, f in enumerate(findings[:50], 1):  # Limit to 50 for summary
            vuln_type = html.escape(f.get("type", "Unknown"))
            severity = f.get("severity_label", f.get("severity", "MEDIUM")).upper()
            url = html.escape(f.get("url", "N/A")[:60])
    ...
    def _generate_finding_block(self, index: int, finding: Dict) -> str:
        ...
        remediation = html.escape(finding.get("remediation", "Consult security best practices."))
...
# Singleton
_generator: Optional[PDFReportGenerator] = None
