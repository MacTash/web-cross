...
<root>&xxe;</root>''',
    ...
    def _make_request(self, url: str, xml_data: str) -> Optional[requests.Response]:
        try:
            headers = {
    ...
    def _check_xxe_response(self, response: requests.Response, 
                            payload: Dict, xml: str) -> Optional[Dict]:
    ...
    def _check_xxe_errors(self, response: requests.Response, payload: Dict) -> Optional[Dict]:
        ...
        for pattern in self.error_patterns:
            if re.search(pattern, content, re.IGNORECASE):
                return {
                    "type": "XXE_ERROR",
                    "payload_name": payload["name"],
                    "evidence": f"XML processing error detected",
                    "confidence": "MEDIUM"
                }
